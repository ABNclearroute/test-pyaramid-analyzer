...
class CSharpPlugin(LanguagePlugin):
    ...
    _WEB_APP_FACTORY = re.compile(r"WebApplicationFactory|TestServer|IClassFixture<Web|HttpClient\s+_client")
    ...
    _PLAYWRIGHT = re.compile(r"using Microsoft\.Playwright|IPage\s+\w+|IBrowser\s+\w+|IPlaywright")
    _SPECFLOW = re.compile(r"using TechTalk\.SpecFlow|using Reqnroll|\[Binding\]|\[Given\]|\[When\]|\[Then\]")

    # Unit-only markers
    _MOCK_FRAMEWORK = re.compile(r"using Moq|using NSubstitute|using FakeItEasy|\.Setup\(|\.Returns\(|Substitute\.For<")
