...
class PhpPlugin(LanguagePlugin):
    ...
    _DB_TEST = re.compile(r"Illuminate\\Foundation\\Testing\\|PHPUnit\\DbUnit\\|extends DatabaseTestCase")
    ...
    def extra_signals(self, file_path: Path, content: str, rules: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if self._REFRESH_DB.search(content) or self._HTTP_TEST.search(content) or self._DB_TEST.search(content):
            signals.append({
